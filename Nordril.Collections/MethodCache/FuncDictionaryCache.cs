...
{
    /// <summary>
    /// A cache of typed constructors for <see cref="FuncDictionary{TKey, TValue}"/>. This is useful if you have value-level types (<see cref="Type"/>) and dynamically want to call the <see cref="FuncDictionary{TKey, TValue}"/>-constructor with them as type argument.
    ...
    /// </summary>
    public class FuncDictionaryCache : DictionaryBasedCache<(Type, Type), Func<IEnumerable<(object, object)>, object>>
    {
        ...
        private static readonly Func<Type, Type, Func<IEnumerable<(object, object)>, object>> makeCreateMethod = (tkey, tvalue) =>
        {
            ...
            var dictType = typeof(FuncDictionary<,>).MakeGenericType(tkey, tvalue);
        ...
        public FuncDictionaryCache(int cacheSize) : base(cacheSize)
        {
        ...
        public IDictionary<TKey, TValue> RetrieveOrCacheCreate<TKey, TValue>(IEnumerable<(TKey, TValue)> elements)
            => (FuncDictionary<TKey, TValue>)RetrieveOrCacheCreate(typeof(TKey), typeof(TValue), elements.Select(e => ((object)e.Item1, (object)e.Item2)));

        /// <summary>
        /// Retrieves <see cref="FuncDictionary{TKey, TValue}"/>-constructor-method and caches both both methods for type <paramref name="key"/> and <paramref name="value"/>.
        /// </summary>
        /// <param name="elements">The list of elements.</param>
