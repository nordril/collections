...
namespace Nordril.Collections.Tests.MethodCache
{
    public class DictionaryCacheTests
    {
        ...
        public static void IntIntDictCreation((int, int)[] xs)
        {
            ...
            var cache = new DictionaryCache(1);
            var actual = (Dictionary<int, int>)cache.RetrieveOrCacheCreate(typeof(int), typeof(int), xs.Select(x => ((object)x.Item1, (object)x.Item2)));
        ...
        public static void StringStringDictCreation((string, string)[] xs)
        {
            var expected = xs.ToDictionary(x => x.Item1, x => x.Item2);
            var cache = new DictionaryCache(1);
            var actual = (Dictionary<string, string>)cache.RetrieveOrCacheCreate(typeof(string), typeof(string), xs.Select(x => ((object)x.Item1, (object)x.Item2)));
        ...
        public static void StringIntDictCreation((string, int)[] xs)
        {
            var expected = xs.ToDictionary(x => x.Item1, x => x.Item2);
            var cache = new DictionaryCache(1);
            var actual = (Dictionary<string, int>)cache.RetrieveOrCacheCreate(typeof(string), typeof(int), xs.Select(x => ((object)x.Item1, (object)x.Item2)));
